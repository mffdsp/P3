...
public class Main {
    ...
    private static int[] indexSAVE = new int[1000];
    private static int[] codeusedSAVE = new int[1000];
    private static String[] DATAnameSAVE = new String[1000];
    private static int[] DATAagendadaySAVE = new int[1000];
    private static String[] DATAagendaweekSAVE = new String[1000];
    private static String[] DATAaddressSAVE = new String[1000];
    private static double[] DATAsalarySAVE = new double[1000];
    private static boolean[] DATAsindicatySAVE = new boolean[1000];
    private static String[] DATAcodeSAVE = new String[1000];
    private static String[] DATAsindicatycSAVE = new String[1000];
    private static double[] DATAsindtaxSAVE = new double[1000] ;
    private static String[] DATApaymodeSAVE = new String[1000];
    private static int[] DATAworktimeSAVE = new int[1000];
    private static String[] DATAtypeSAVE = new String[1000];
    private static int[] AGENDAboolSAVE = new int[1000];


    //UNDE REDO
    private static int TOTALACTIONS = 0;
    private static int SAVEINDEX = 0;
    private static void saveState(int pos){
        ...
        if(SAVEINDEX == 0)
        {
            ...
            codeusedSAVE[SAVEINDEX] = 0;
            indexSAVE[SAVEINDEX] = pos;
            return;
        }
        indexSAVE[SAVEINDEX] = pos;


        System.out.println("INDEX = " + SAVEINDEX);
        codeusedSAVE[SAVEINDEX] = codeused[indexSAVE[SAVEINDEX]];
        DATAnameSAVE[SAVEINDEX] = DATAname[indexSAVE[SAVEINDEX]];
        DATAagendaweekSAVE[SAVEINDEX] = DATAagendaweek[indexSAVE[SAVEINDEX]];
        DATAaddressSAVE[SAVEINDEX] = DATAaddress[indexSAVE[SAVEINDEX]];
        DATAsindicatySAVE[SAVEINDEX] = DATAsindicaty[indexSAVE[SAVEINDEX]];
        DATAsalarySAVE[SAVEINDEX] =  DATAsalary[indexSAVE[SAVEINDEX]];
        DATAcodeSAVE[SAVEINDEX] = DATAcode[indexSAVE[SAVEINDEX]];
        DATAsindicatycSAVE[SAVEINDEX] = DATAsindicatyc[indexSAVE[SAVEINDEX]];
        DATAsindtaxSAVE[SAVEINDEX] = DATAsindtax[indexSAVE[SAVEINDEX]];
        DATAworktimeSAVE[SAVEINDEX] = DATAworktime[indexSAVE[SAVEINDEX]];
        DATAtypeSAVE[SAVEINDEX] =  DATAtype[indexSAVE[SAVEINDEX]];
    ...
    private static void undo(){
        ...
        }else
        {
            ...
            codeused[indexSAVE[SAVEINDEX]] = codeusedSAVE[SAVEINDEX];

            DATAname[indexSAVE[SAVEINDEX]] = DATAnameSAVE[SAVEINDEX];
            DATAagendaweek[indexSAVE[SAVEINDEX]] = DATAagendaweekSAVE[SAVEINDEX];
            DATAaddress[indexSAVE[SAVEINDEX]] = DATAaddressSAVE[SAVEINDEX];
            DATAsindicaty[indexSAVE[SAVEINDEX]] = DATAsindicatySAVE[SAVEINDEX];
            DATAsalary[indexSAVE[SAVEINDEX]] = DATAsalarySAVE[SAVEINDEX];
            DATAcode[indexSAVE[SAVEINDEX]] = DATAcodeSAVE[SAVEINDEX];
            DATAsindicatyc[indexSAVE[SAVEINDEX]] = DATAsindicatycSAVE[SAVEINDEX];
            DATAsindtax[indexSAVE[SAVEINDEX]] = DATAsindtaxSAVE[SAVEINDEX];
            DATAworktime[indexSAVE[SAVEINDEX]] =  DATAworktimeSAVE[SAVEINDEX];
            DATAtype[indexSAVE[SAVEINDEX]] = DATAtypeSAVE[SAVEINDEX];
    ...
    private static void redo(){
        ...
        if(TOTALACTIONS < SAVEINDEX)
        {
            System.out.println("Sem açao disponivel para refazer");
        }else{

            SAVEINDEX += 1;
            System.out.println("Ultima Acao Refeita com sucesso! " + SAVEINDEX);
            codeused[indexSAVE[SAVEINDEX]] = codeusedSAVE[SAVEINDEX];

            System.out.println( codeused[indexSAVE[SAVEINDEX]] + " " + SAVEINDEX + " " + indexSAVE[SAVEINDEX]);
